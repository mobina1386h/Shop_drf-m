...
class OrderField(models.PositiveIntegerField):
    description="Ordering Field For Unique Product"
    ...
    def check(self, **kwargs):
        return [
            *super().check(**kwargs),*self._check_unique_for_field(**kwargs)
        ]
    ...
    def _check_unique_for_field(self, **kwargs):
        ...
        if self.unique_for_field is None:
            return [
                checks.ERROR("OrderField must defind",obj=self,id="product.E001")
            ]

        elif self.unique_for_field not in field_list:
            return [
                checks.ERROR("OrderField not find",obj=self,id="product.E002")
            ]
        else:
            return []
